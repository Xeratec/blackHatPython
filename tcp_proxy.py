...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
	...
	# recieve data from remote host if necessary
	if receive_first:
		
		remote_buffer = recieve_from(remote_socket)
		hexdump(remote_buffer)
		
		# send it to our response handerl
		remote_buffer = response_handler(remote_buffer)
		
		# if we hava data to send to our local client, send it
		...
		while True:
			...
			if len(local_buffer):
				...
				print "[==>] Send %d bytes to remote." % len(local_buffer)
				
			# recieve back to respinse
			remote_buffer = recieve_from(remote_socket)
			...
			if len(remote_buffer):
				...
				# send data to remote host
				client_socket.send(remote_buffer)
				print "[==>] Send %d bytes to localhsot." % len(remote_buffer)	
				
			# if no more data on eihter side close connetion
...
def recieve_from(connection):
	...
	# We set a 2 second timeout; depening on the target this may need to be adjusted
